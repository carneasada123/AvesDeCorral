...
<head>
    ...
    <title>Laravel 10 con Bootstrap</title>
...
<img class="background" src="./images/gallo-fondo.jpg" alt="">
    ...
    document.getElementById('loginForm').addEventListener('submit', async function (event) {
        ...
        try {
            ...
            if (response.ok) {
                // Guardar los tres tokens en localStorage
                localStorage.setItem('login_token', data.login_token);
                localStorage.setItem('role_token', data.role_token);
                localStorage.setItem('status_token', data.status_token);

                toastr.success("Inicio de sesión exitoso!");

                const rolePayload = jwt_decode(data.role_token);
                const userRole = rolePayload.role;
                ...
                if (userRole === 1) {
                    ...
                    window.location.assign('/almacen');
                } else {
                    toastr.error("Rol desconocido: " + userRole);
                }
            } else {
                toastr.error(data.error || 'Error al iniciar sesión');
            }

        } catch (error) {
            console.error('Error en el frontend:', error);
            toastr.error('Ocurrió un error inesperado en el frontend');
        }
    });
</script>
